#!/usr/bin/env python

# Python core libraries
import sys
import logging
from functools import partial

# Librairies files
from PyQt5.QtWidgets import QMainWindow, QApplication
from PyQt5.QtCore import QThread, Qt
from PyQt5.QtGui import QIcon, QMovie
from tendo import singleton

# Window file generated by QT Designer
from gui.window_ui import Ui_MainWindow
from gui.notification import Notification
from gui.preferences import PreferencesDialog

from user import User
from config import assets
from trayIcon import TrayIcon

if __debug__:
    from gui.logger_ui import LoggerDialog

class MainWindow(QMainWindow, Ui_MainWindow):
    closeWindow = False

    def __init__(self, app):
        super(MainWindow, self).__init__()
        self.app = app
        self.setupUi(self)

        icon = QIcon(assets['nautiljon_icon'])
        self.systemTrayIcon = TrayIcon(self, icon);
        self.setWindowIcon(icon)

        self.workerThread = QThread()
        self.user = User()
        self.user.moveToThread(self.workerThread)
        self.user.onFinishedConnect.connect(self.onFinishedConnect)

        # Center window
        screenRect = QApplication.desktop().screenGeometry()
        self.setGeometry(
            (screenRect.width() / 2) - (self.width() / 2),
            (screenRect.height() / 2) - (self.height() / 2),
            self.width(),
            self.height()
        )

        # Init spinner
        self.loaderMovie = QMovie(assets['loading_spinner'])
        self.loadingSpinner.setMovie(self.loaderMovie)
        self.displaySpinner(True)

    def initUser(self):
        # Init user in thread
        self.workerThread.quit()
        self.workerThread.started.connect(partial(self.user.init, self))
        self.workerThread.start()

    def cleanUp(self):
        Notification.cleanUp()

        self.user.cleanUp()
        self.workerThread.quit()
        self.workerThread.wait()

        # Hide the system tray icon or the user has to pass his mouse over
        # the icon so it disappear
        self.systemTrayIcon.hide()

    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Return:
            self.onConnect(True)

    def onActionQuit(self, checked):
        self.closeWindow = True
        self.close()

    def onActionPreferences(self, checked):
        preferencesDialog = PreferencesDialog(self, self.user)
        preferencesDialog.show()

    def closeEvent(self, event):
        # Don't close window if user exit window
        # Only quit if the "quit" option has been clicked in the system tray icon
        if not self.closeWindow:
            event.ignore()
            self.hide()
        # Close window and quit application
        else:
            self.close()
            self.cleanUp()
            self.app.quit()

    def displayLoginForm(self, display, errorMessage = None):
        self.displaySpinner(False)

        if display:
            self.loginForm.show()
            self.connectSuccess.hide()
            self.connectError.hide()

            if errorMessage:
                self.connectError.setText(errorMessage)
                self.connectError.show()
        else:
            self.loginForm.hide()
            self.connectError.hide()
            self.connectSuccess.show()

            # Empty inputs
            self.usernameInput.setText("")
            self.passwordInput.setText("")


    def displaySpinner(self, display):
        if display:
            self.formContainer.hide()
            self.loadingSpinner.show()
            self.loaderMovie.start()
        else:
            self.formContainer.show()
            self.loaderMovie.stop()
            self.loadingSpinner.hide()

    def onNewNotification(self, notification):
        logging.debug('Window received notification')
        Notification.create(
            notification['title'],
            notification['message'],
            notification['pixmap'],
            notification['url'],
            notification.get('onClick'),
            self.user.settings.notificationsCloseSeconds * 1000
        )

    def onUserDisconnected(self):
        logging.debug('Window received user deconnection')

        self.user.connected = False
        self.displayLoginForm(True)

        self.systemTrayIcon.showMessage('Nautiljon Notifier', 'Attention, vous n\'êtes plus connecté', QIcon(assets['nautiljon_icon']))

    def onFinishedConnect(self, errMsg, success):
        self.workerThread.exit()
        self.displayLoginForm(not success, errMsg)

    def onConnect(self, checked):
        if self.workerThread.isRunning():
            return

        logging.debug('Connect button clicked')
        username = self.usernameInput.text()
        password = self.passwordInput.text()

        self.displaySpinner(True)
        self.workerThread.quit()

        # Disconnect all bindings
        try:
            self.workerThread.started.disconnect()
        # Throw an exception if no functions binded
        except Exception as e:
            pass

        self.workerThread.started.connect(partial(self.user.connect, username, password))
        self.workerThread.start()

def main():
    app = QApplication(sys.argv)
    # Set this option or the app will close
    # if a notification is closed when the window is hidden
    app.setQuitOnLastWindowClosed(False)

    main_window = MainWindow(app)
    main_window.menuBar.setNativeMenuBar(False)

    if __debug__:
        logger_window = LoggerDialog(main_window)
        logger_window.move(0, 0)
        logger_window.setGeometry(0, 0, 1000, 300)
        logger_window.show()

    main_window.show()
    main_window.initUser()

    ret = app.exec_()
    sys.exit(ret)

if __name__ == "__main__":
    # Tendo library make the app crash on exit on linux
    # Only use on windows
    if sys.platform == 'win32' or sys.platform == 'cygwin':
        # Will sys.exit(-1) if other instance is running
        instance = singleton.SingleInstance()
    main()
